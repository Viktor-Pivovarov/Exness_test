...
class StockQuote:
    ...
    # Set the stock symbols after performing necessary validation
    ...
    def get_quote(self):
        # Convert the list of symbols into a string for the API call
    ...
    def export_plot(self, rows, symbol):
        ...
        # Convert 'latestUpdate' from Unix Time Stamp to normal date/time
        ...
        plt.savefig(f'{symbol}_data_{dt_string}.png')
    ...
    def export_to_csv(self, rows, symbol):
        ...
        now = datetime.now()
        dt_string = now.strftime("%d_%m_%Y_%H_%M_%S")
        df.to_csv(f'{symbol}_data_{dt_string}.csv')
...
def main():
    ...
    while True:
        ...
        symbols = input("Enter stock symbol or symbols separated by comma and press Enter: ").split(',')
        ...
        if sq.set_symbols(symbols):
            ...
            if data:
                sq.save_to_db(data)
                for symbol in symbols:
                    rows = sq.get_from_db(symbol.strip())
                    if rows:
